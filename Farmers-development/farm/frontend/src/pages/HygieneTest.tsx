...
const HygieneTest = () => {
  ...
  const [photoUploads, setPhotoUploads] = useState<{ poultry: File[]; cattle: File[] }>({
    poultry: [],
    cattle: []
  });
  ...
  const getRiskLevel = (score: number) => {
    ...
    if (score >= 45) return { level: t('moderate'), color: 'text-yellow-600', message: t('moderateMsg') };
  ...
  const resetTest = () => {
    ...
    setPhotoUploads({ poultry: [], cattle: [] });
  };
  ...
  const handleRemovePhoto = (index: number) => {
    ...
    setPhotoUploads((prev) => ({
      ...prev,
      [farmType]: prev[farmType].filter((_, i) => i !== index)
    }));
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 pt-24 pb-12 max-w-3xl">
        <Card className="mb-6">
          <CardHeader>
            <CardTitle className="text-2xl">{t('uploadPhotosTitle')}</CardTitle>
            <CardDescription>
              {farmType === 'poultry' ? t('uploadPhotosDescriptionPoultry') : farmType === 'cattle' ? t('uploadPhotosDescriptionCattle') : t('selectFarmType')}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              {farmType && photoUploads[farmType].length > 0 ? (
                <ul className="space-y-2">
                  {photoUploads[farmType].map((file, index) => (
                    <li key={`${file.name}-${index}`} className="flex items-center justify-between rounded-md border px-3 py-2 text-sm">
                      <span className="truncate max-w-[60%]">{file.name}</span>
                      <Button variant="ghost" size="sm" onClick={() => handleRemovePhoto(index)}>
                        {t('remove')}
                      </Button>
                    </li>
                  ))}
